...
class VistaPropuestasAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PropuestaHolder {
        val view =  LayoutInflater.from(parent.context).inflate(R.layout.item_propuesta,parent,false)
    ...
    override fun onBindViewHolder(holder: PropuestaHolder, position: Int) {
        Glide
            .with(context)
            .load("https://cdn2.iconfinder.com/data/icons/coronavirus-outbreak-2/275/Recurso_6-256.png")
            .centerInside()
            .into(holder.getImageView());

        holder.setDireccion(StringBuilder().append(propuestas[position].id).toString())
